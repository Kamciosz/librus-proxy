...
app.post("/debug-auth", async (req, res) => {
    ...
    try {
        ...
        trace.hypothesis = "/Grant (hardcoded) jednym requestem z followRedirect → synergia?code=XXX → authenticated DZIENNIKSID";

        const grantUrlHardcoded = `${OAUTH_BASE}/OAuth/Authorization/Grant?client_id=46`;

        // JEDEN request z pełnym śledzeniem redirect
        const r3 = await client.get(grantUrlHardcoded, { 
